<?
  ...
  $maxValue = abs($cash_flow_data[0]["NOI_VALUE"]);
  foreach($cash_flow_data as $v) {
     $graph_data["ADDRESS1"][]  = $v["ADDRESS1"];
     $graph_data["NOI_VALUE"][]  = $v["NOI_VALUE"];
     if (abs($v["NOI_VALUE"]) > $maxValue)
        $maxValue = abs($v["NOI_VALUE"]);
  }
  ...
  $b1 = new BarPlot($graph_data["NOI_VALUE"]);
