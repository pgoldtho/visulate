...
Is
  ...
  *           the SQL that doesnt use SDO_AGGR_MBR and comment out the SDO_AGGR_MBR SQL.
  ...
  *           If not use this packages ConvertGeometry() function.
  ...
  * @precis     : Function that generates a series of numbers mimicking PostGISs function with
  *               the same name
  * @version    : 1.0
  ...
  *               not mentioned in Oracles licensing as being functions limited to Spatial (EE)
  ...
  **/
  ...
  /** ----------------------------------------------------------------------------------------
  * @function   : GetVector
  * @precis     : Places a geometrys coordinates into a pipelined vector data structure.
  * @version    : 3.0
  ...
  * @param      : p_geometry     : MDSYS.SDO_GEOMETRY : The geometry object.
  * @param      : p_start        : pls_integer : 0 = Use average of all Areas vertices for starting X centroid calculation
  ...
  **/
  FUNCTION centroid_a(p_geometry IN mdsys.sdo_geometry,
                      ...
                      p_round_x  IN pls_integer := 3,
                      p_round_y  IN pls_integer default NULL)
    RETURN MDSYS.SDO_GEOMETRY DETERMINISTIC;
  ...
  * @return     : centroid       : MDSYS.SDO_GEOMETRY : The centroid.
  * @requires   : GetVector()
  * @history    : Simon Greener - Mar 2008 - Total re-write of algorithm following on from cases the original algorithm didnt handle.
  ...
  *                                          rather than the mid point of a parts MBR. This is to try and "weight" the centroid more towards where detail exists.
  ...
  * @param      : p_start    : Number : 0 = Use average of all Areas vertices for starting X centroid calculation
  *                                     1 = Use centre X of MBR
...
AS
  ...
  Begin
    return case when p_dims = 2
                then MDSYS.SDO_DIM_ARRAY(MDSYS.SDO_DIM_ELEMENT('X', c_MinVal, c_MaxVal, p_tolerance),
                                         MDSYS.SDO_DIM_ELEMENT('Y', c_MinVal, c_MaxVal, p_tolerance))
                when p_dims = 3
                then MDSYS.SDO_DIM_ARRAY(MDSYS.SDO_DIM_ELEMENT('X', c_MinVal, c_MaxVal, p_tolerance),
                                         MDSYS.SDO_DIM_ELEMENT('Y', c_MinVal, c_MaxVal, p_tolerance),
                                         MDSYS.SDO_DIM_ELEMENT('Z', c_MinVal, c_MaxVal, p_tolerance))
            end;
  ...
  Is
  BEGIN
     RETURN CENTROID.ConvertGeometry(p_geometry,p_Arc2Chord);
  END SDO_ARC_DENSIFY;
...
*/
  ...
  AS
    ...
    v_seed            MDSYS.SDO_GEOMETRY;
    v_centroid        MDSYS.SDO_GEOMETRY;
    v_round_x         pls_integer;
    v_round_y         pls_integer;
    NULL_GEOMETRY     EXCEPTION;
    NULL_ROUND_FACTOR EXCEPTION;
    STARTING_POSITION EXCEPTION;
    NO_SEED_X         EXCEPTION;
    SEED_MBR          EXCEPTION;
  Begin
    If ( p_geometry is NULL ) Then
       raise NULL_GEOMETRY;
    End If;
    If ( p_round_x is NULL ) Then
       raise NULL_GEOMETRY;
    End If;
    v_round_x := p_round_x;
    v_round_y := NVL(p_round_y,v_round_x);
    -- Get MBR of polygon
    IF ( p_start NOT IN (0,1,2) ) THEN
       raise STARTING_POSITION;
    END IF;
    If ( p_start = 2 AND p_seed_x IS NULL ) THEN
       raise NO_SEED_X;
    END IF;
    ...
    --
	  v_seed := CASE p_start
	                 WHEN 0 THEN p_geometry
				           WHEN 1 THEN &&defaultSchema..CENTROID.sdo_mbr(p_geometry)
		               WHEN 2 THEN mdsys.sdo_geometry(2001,p_geometry.SDO_SRID,MDSYS.SDO_POINT_TYPE(p_seed_x,0,NULL),NULL,NULL)
				           ELSE p_geometry
				       END;
    ...
    IF ( p_start = 2 ) THEN
       ...
       IF ( p_seed_x <= v_mbr.sdo_ordinates(1) OR p_seed_x >= v_mbr.sdo_ordinates(3) ) THEN
          raise SEED_MBR;
       END IF;
    END IF;
                                      ...
                                      FROM (SELECT t.element_id, t.subelement_id,
                                              ...
                                              FROM (SELECT r.element_id, r.subelement_id,
                                                           ...
                                                           case when (r.endx = r.startx)
                                                                ...
                                                                else round(r.starty + ( (r.endy-r.starty)/(r.endx-r.startx) ) * (r.x-r.startx),v_round_x)
                                                            end as y,
                                                        ...
                                                        FROM (SELECT v.element_id, v.subelement_id, c.x,
                                                                     round(v.startCoord.x,v_round_x) as startX,
                                                                     round(v.startCoord.y,v_round_y) as startY,
                                                                     round(  v.endCoord.x,v_round_x) as endX,
                                                                     round(  v.endCoord.y,v_round_y) as endY
                                                                FROM (SELECT round(avg(p.x),v_round_x) as x
                                                                        FROM TABLE(mdsys.sdo_util.GetVertices(v_seed)) p
                                                                     ) c,
    ...
    EXCEPTION
      WHEN NO_DATA_FOUND Then
           raise_application_error(c_i_paracentroid, c_s_paracentroid,TRUE);
           RETURN NULL;
      WHEN NULL_GEOMETRY Then
           raise_application_error(c_i_null_geometry, c_s_null_geometry,TRUE);
           RETURN NULL;
      WHEN NULL_ROUND_FACTOR Then
           raise_application_error(c_i_null_round_factor, c_s_null_round_factor,TRUE);
           RETURN NULL;
      WHEN STARTING_POSITION THEN
           raise_application_error(-20001,'Starting position must be 0, 1 or 2.',TRUE);
           RETURN NULL;
      WHEN NO_SEED_X THEN
           raise_application_error(-20001,'Seed X value not provided.',TRUE);
           RETURN NULL;
      WHEN SEED_MBR THEN
          raise_application_error(-20001,'Seed X value not between provided geometry''s MBR.',TRUE);
          RETURN NULL;
      WHEN OTHERS THEN
           raise_application_error(-20001,SQLERRM,TRUE);
           RETURN NULL;
                      ...
                      FROM (SELECT rownum as rid,
                                   ...
                                   mdsys.sdo_geom.sdo_distance(
                                         mdsys.sdo_geometry(2001,
                                                            a.srid,
                                                            mdsys.sdo_point_type(v.startCoord.X,v.startCoord.y,v.startCoord.z),
                                                            NULL,
                                                            NULL),
                                         mdsys.sdo_geometry(2001,
                                                            a.srid,
                                                            mdsys.sdo_point_type(v.endCoord.X,v.endCoord.y,v.endCoord.z),
                                                            NULL,
                                                            NULL),
                                         1/power(10,p_round_x),
                                         CASE when a.srid IS NULL THEN NULL ELSE 'unit=m' END
                                   ) as vectorLength
                              FROM (SELECT p_geometry.sdo_srid as srid,
                                           CASE WHEN g_spatial = 0
                                                THEN &&defaultSchema..CENTROID.sdo_length(
                                                                               p_geometry,
                                                                               1/power(10,p_round_x),
                                                                               CASE WHEN p_geometry.sdo_srid IS NULL THEN NULL ELSE 'METER' END)
                                                ELSE mdsys.sdo_geom.sdo_length(p_geometry,
                                                                               1/power(10,p_round_x),
                                                                               CASE WHEN p_geometry.sdo_srid IS NULL THEN NULL ELSE 'unit=m' END)
  ...
  Begin
       ...
       FROM (SELECT a.geometry
              ...
              ORDER BY 99999999999999999 +
                       ...
                       CASE WHEN v_gtype = 6 /* linestring */
                            THEN CASE WHEN g_spatial = 0
                                      THEN &&defaultSchema..CENTROID.sdo_length(a.geometry,1/power(10,p_round_x))
                                      ELSE mdsys.sdo_geom.sdo_length(a.geometry,1/power(10,p_round_x))
                                  END/* g_spatial */
                            ELSE /* v_gtype = 7 POLYGON */
                                 CASE WHEN g_spatial = 0
                                      THEN CASE WHEN p_start = 1
                                                THEN &&defaultSchema..CENTROID.sdo_area(a.geometry,1/power(10,p_round_x))
                                                ELSE &&defaultSchema..CENTROID.sdo_area(&&defaultSchema..CENTROID.sdo_mbr(a.geometry),1/power(10,p_round_x))
                                            END /* p_area */
                                      ELSE CASE WHEN p_start = 1
                                                THEN mdsys.sdo_geom.sdo_area(a.geometry,1/power(10,p_round_x))
                                                ELSE mdsys.sdo_geom.sdo_area(&&defaultSchema..CENTROID.sdo_mbr(a.geometry),1/power(10,p_round_x))
                                            END /* p_area */
                                  END /* g_spatial */
                        END /* v_gtype = 6 */
            )
    ...
    CASE WHEN v_gtype IN (1,5)
         THEN v_geometry := centroid_p(v_geometry,p_round_x,p_round_y,p_round_z);
         WHEN v_gtype in (2,6)
         THEN v_geometry := centroid_l(v_geometry,0.5,p_round_x,p_round_y,p_round_z);
         WHEN v_gtype in (3,7)
         THEN v_geometry := centroid_a(v_geometry,p_start,NULL,p_round_x,p_round_y);
         ELSE v_geometry := NULL;
     END CASE;
  ...
  Begin
    ...
    END IF;
    RETURN sdo_Centroid(p_geometry,
                        p_start,1,
                        v_x_round_factor,
                        v_y_round_factor,
  ...
  Begin
    ...
    IF ( p_dimarray.count > 2 ) Then
      v_z_round_factor := floor(log(10,(1/p_dimarray(3).sdo_tolerance)/2));
    END IF;
    RETURN sdo_Multi_Centroid(p_geometry,
                              p_start,
                              v_x_round_factor,
                              v_y_round_factor,
                              case when p_dimarray.COUNT=3 then v_z_round_factor else null end);
