...
CREATE UNIQUE INDEX RNT_USER_ASSIGNMENTS_U2 ON RNT_USER_ASSIGNMENTS
(ROLE_ID, USER_ID)
LOGGING
NOPARALLEL;

ALTER TABLE RNT_ACCOUNTS_RECEIVABLE
 ADD (SOURCE_FOR_LATE_FEE  NUMBER);

ALTER TABLE RNT_ACCOUNTS_RECEIVABLE
 ADD (AGREEMENT_ACTION_ID  NUMBER);

ALTER TABLE RNT_ACCOUNTS_RECEIVABLE
 ADD (PAYMENT_PROPERTY_ID  NUMBER);


COMMENT ON COLUMN RNT_ACCOUNTS_RECEIVABLE.SOURCE_FOR_LATE_FEE IS 'Source ID for late fee payment. Reserved';

COMMENT ON COLUMN RNT_ACCOUNTS_RECEIVABLE.AGREEMENT_ACTION_ID IS 'FK to RNT_AGREEMENT_ACTIONS';

COMMENT ON COLUMN RNT_ACCOUNTS_RECEIVABLE.PAYMENT_PROPERTY_ID IS 'FK to RNT_PROPERTIES';
CREATE UNIQUE INDEX RNT_TENANT_U1 ON RNT_TENANT
...
(AR_ID, PAYMENT_DUE_DATE, AMOUNT, PAYMENT_TYPE, TENANT_ID, 
 ...
 PAYMENT_PROPERTY_ID, CHECKSUM, TENANT_NAME, PROPERTY_ID, RECEIVABLE_TYPE_NAME)
AS 
select ar.AR_ID
,      ar.PAYMENT_DUE_DATE
,      ar.AMOUNT
...
where t.TENANT_ID = ar.TENANT_ID
  ...
  and a.AGREEMENT_ID = ar.AGREEMENT_ID
...
-------------------------------------------------
  ...
  begin
     ...
     -- not discount or late fee 
     if p_new_type = 7 then
        return p_amount;
     elsif p_new_type = 10 then   
        return p_amount + p_discount_amount;
     elsif p_new_type = 8 then     
        return p_amount - p_discount_amount;
     end if; 
  ...
  begin
    ...
    */
    ...
    for x in c loop
      ...
      if x.LATE_FEE_AR_ID is not null and x_new_type = 10 then 
         null;
      else                            
        if x_new_type != x.PAYMENT_TYPE then
        
          if x.PAYMENT_TYPE = 7 then
             if x_new_type = 8 then
                   -- discount rent
                   update RNT_ACCOUNTS_RECEIVABLE
                   set AMOUNT = x.AMOUNT - x.DISCOUNT_AMOUNT,
                       PAYMENT_TYPE = 8
                   where AR_ID = x.AR_ID;                                
             elsif x_new_type = 10 then
                   update RNT_ACCOUNTS_RECEIVABLE
                   set AMOUNT = x.AMOUNT + x.DISCOUNT_AMOUNT,
                       PAYMENT_TYPE = 10
                   where AR_ID = x.AR_ID;
          ...
          if x.PAYMENT_TYPE = 8 then
            if x_new_type = 7 then
               -- rent
               update RNT_ACCOUNTS_RECEIVABLE
               set AMOUNT = x.AMOUNT,
                   PAYMENT_TYPE = 7
           ...
           if x.PAYMENT_TYPE = 10 then
               if x_new_type in (7, 8) then
                   -- rent
                   /*
                   delete from RNT_ACCOUNTS_RECEIVABLE
...
begin
  ...
  for x in (select TENANT_ID
            from RNT_TENANT t
            where t.AGREEMENT_ID = X_AGREEMENT_ID ) loop
...
ALTER TABLE RNT_ACCOUNTS_RECEIVABLE
 ADD CONSTRAINT RNT_ACCOUNTS_RECEIVABLE_FK5 
 FOREIGN KEY (AGREEMENT_ACTION_ID) 
 REFERENCES RNT_AGREEMENT_ACTIONS (ACTION_ID);

ALTER TABLE RNT_ACCOUNTS_RECEIVABLE
 ADD CONSTRAINT RNT_ACCOUNTS_RECEIVABLE_FK2 
