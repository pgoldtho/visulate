...
is
   cursor c is
        ...
        from (
              ...
              (
                select trunc(payment_due_date, 'MM') as MONTH_DATE, sum(ap.amount)  EXPENSE_AMOUNT
   ...
   cursor c is
        ...
        from RNT_PROPERTIES p,
             ...
             (  select p.property_id, sum(ap.amount) property_costs_amount
                ...
                and pt.payment_type_name != 'Mortgage Payment'
...
is
  cursor c_agr_expiried is
   ...
   cursor c_vacant is
        select p.ADDRESS1, p.PROPERTY_ID, p.BUSINESS_ID
        from RNT_PROPERTIES p
        where BUSINESS_ID = X_BUSINESS_ID
          and DATE_SOLD is null 
          and not exists (select 1
                          from  RNT_PROPERTY_UNITS pu,
