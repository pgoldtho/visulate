...
<li><b>Phone:</b> <a href="tel:+1-321-453-7389">(321) 453 7389</a></li>
  ...
  begin
    for c_rec in cur_county_list loop
      ...
      v_text := '<h3>'||c_rec.name||' Real Estate - '||c_rec.city||'</h3>'
                      ||get_static_text(c_rec.ucity);
      v_text := v_text||get_visulate_ad(c_rec.city);
      v_len := length(v_text);
      if v_len > 0 then
    ...
    for c_rec in
      ...
      dbms_lob.createtemporary(v_desc, TRUE);
      dbms_lob.open(v_desc, dbms_lob.lob_readwrite);
      v_text := '<h3>'||initcap(c_rec.county)||' County Real Estate - '||initcap(c_rec.name)||'</h3>';
      v_text := v_text||get_visulate_ad(initcap(c_rec.name));
