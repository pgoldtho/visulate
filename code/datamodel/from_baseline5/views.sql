...
--   Script Created at  : 28.05.2007 22:14:32 
--   Physical Location  :  
--   Notes              :  
--

-- Object Counts: 
--   Views: 17          Columns: 197        


--
-- RNT_ACCOUNTS_PAYABLE_V  (View) 
--
CREATE OR REPLACE FORCE VIEW RNT_ACCOUNTS_PAYABLE_V
(AP_ID, PAYMENT_DUE_DATE, AMOUNT, PAYMENT_TYPE_ID, EXPENSE_ID, 
 LOAN_ID, SUPPLIER_ID, CHECKSUM, SUPPLIER_NAME)
AS 
select ap.AP_ID
,      ap.PAYMENT_DUE_DATE
,      ap.AMOUNT
...
,      ap.LOAN_ID
,      ap.SUPPLIER_ID
,      rnt_sys_checksum_rec_pkg.get_checksum('AP_ID='||ap.AP_ID||'PAYMENT_DUE_DATE='||ap.PAYMENT_DUE_DATE||'AMOUNT='||ap.AMOUNT||'PAYMENT_TYPE_ID='||ap.PAYMENT_TYPE_ID||'EXPENSE_ID='||ap.EXPENSE_ID||'LOAN_ID='||ap.LOAN_ID||'SUPPLIER_ID='||ap.SUPPLIER_ID) as CHECKSUM
...
-- RNT_PAYMENTS_V  (View) 
--
CREATE OR REPLACE FORCE VIEW RNT_PAYMENTS_V
(PAYMENT_ID, PAYMENT_DATE, DESCRIPTION, PAID_OR_RECEIVED, AMOUNT, 
 TENANT_ID, BUSINESS_ID, SUMMARY_ALLOCATED, CHECKSUM)
AS 
select PAYMENT_ID
,      PAYMENT_DATE
,      DESCRIPTION
...
,      (select NVL(sum(AMOUNT), 0)
        ...
        where PAYMENT_ID = p.PAYMENT_ID) as SUMMARY_ALLOCATED  
,      rnt_sys_checksum_rec_pkg.get_checksum('PAYMENT_ID='||PAYMENT_ID||'PAYMENT_DATE='||PAYMENT_DATE||'DESCRIPTION='||DESCRIPTION||'PAID_OR_RECEIVED='||PAID_OR_RECEIVED||'AMOUNT='||AMOUNT||'TENANT_ID='||TENANT_ID||'BUSINESS_ID='||BUSINESS_ID) as CHECKSUM
from RNT_PAYMENTS p
order by PAYMENT_DATE, DESCRIPTION;


--
-- RNT_ACCOUNTS_RECEIVABLE_V  (View) 
--
CREATE OR REPLACE FORCE VIEW RNT_ACCOUNTS_RECEIVABLE_V
(AR_ID, PAYMENT_DUE_DATE, AMOUNT, PAYMENT_TYPE, TENANT_ID, 
 AGREEMENT_ID, LOAN_ID, BUSINESS_ID, IS_GENERATED_YN, CHECKSUM, 
 TENANT_NAME, PROPERTY_ID, RECEIVABLE_TYPE_NAME)
AS 
select ar.AR_ID
,      ar.PAYMENT_DUE_DATE
,      ar.AMOUNT
...
,      ar.BUSINESS_ID
,      ar.IS_GENERATED_YN
,      rnt_sys_checksum_rec_pkg.get_checksum('AR_ID='||ar.AR_ID||'PAYMENT_DUE_DATE='||ar.PAYMENT_DUE_DATE||'AMOUNT='||ar.AMOUNT||'PAYMENT_TYPE='||ar.PAYMENT_TYPE||'TENANT_ID='||ar.TENANT_ID||'AGREEMENT_ID='||ar.AGREEMENT_ID||'LOAN_ID='||ar.LOAN_ID||'BUSINESS_ID='||ar.BUSINESS_ID||'IS_GENERATED_YN='||ar.IS_GENERATED_YN) as CHECKSUM
...
(UNIT_ID, PROPERTY_ID, UNIT_NAME, UNIT_SIZE, BEDROOMS, 
 BATHROOMS, CHECKSUM, BUSINESS_ID)
AS 
select u.UNIT_ID, 
       u.PROPERTY_ID, 
...
--
-- RNT_PAYMENT_ALLOCATIONS_V  (View) 
--
CREATE OR REPLACE FORCE VIEW RNT_PAYMENT_ALLOCATIONS_V
(PAY_ALLOC_ID, PAYMENT_DATE, AMOUNT, AR_ID, AP_ID, 
 PAYMENT_ID, CHECKSUM)
AS 
select PAY_ALLOC_ID
,      PAYMENT_DATE
...
,      AP_ID
,      PAYMENT_ID
,      rnt_sys_checksum_rec_pkg.get_checksum('PAY_ALLOC_ID='||PAY_ALLOC_ID||'PAYMENT_DATE='||PAYMENT_DATE||'AMOUNT='||AMOUNT||'AR_ID='||AR_ID||'AP_ID='||AP_ID||'PAYMENT_ID='||PAYMENT_ID) as CHECKSUM
from RNT_PAYMENT_ALLOCATIONS;
