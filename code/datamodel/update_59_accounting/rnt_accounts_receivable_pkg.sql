 ...
 */
 ...
 function get_pt_rule( x_business_unit   in rnt_business_units.business_id%type
                     , x_payment_type_id in rnt_payment_types.payment_type_id%type)
 return   rnt_pt_rules%rowtype
 is
   cursor cur_accounts is
   select * from rnt_pt_rules
   where  business_id      = x_business_unit
   and    payment_type_id  = x_payment_type_id
   and    transaction_type = 'ARS';
   --
   v_return_value    rnt_pt_rules%rowtype;
 begin
     open  cur_accounts;
     fetch cur_accounts into v_return_value;
     close cur_accounts;
     --
     return v_return_value;
 end;
...
--dbms_output.put_line(v_ar_id ||' - '||v_date);
        ...
        if v_date <= x_effective_date then
          ...
          v_ledger_id := rnt_ledger_entries_pkg.insert_row(
                                 X_ENTRY_DATE      => v_date
                               , X_DESCRIPTION     => 'Scheduled receivable for '||a_rec.address1
                               ...
                               , X_PAYMENT_TYPE_ID => PAYMENT_TYPE_RENT()
                               , X_PROPERTY_ID     => a_rec.property_id);
          ...
          if t_rec.section8_id is not null then
            ...
            v_ledger_id := rnt_ledger_entries_pkg.insert_row(
                                 ...
                                 , X_AP_ID           => to_number(null)
                                 , X_PAY_ALLOC_ID    => to_number(null)
                                 , X_PAYMENT_TYPE_ID => PAYMENT_TYPE_SECTION8_RENT()
                                 , X_PROPERTY_ID     => a_rec.property_id);                                 
          end if;          
        end if;
      end loop;
 ...
 */
 ...
 begin
   if X_PAYMENT_DATE is null then
     delete from RNT_LEDGER_ENTRIES
     where  pay_alloc_id = (select pay_alloc_id from rnt_payment_allocations
                            where  ar_id = X_AR_ID);
     --
     ...
     if SQL%ROWCOUNT = 0 then
         ...
         v_ledger_id := rnt_ledger_entries_pkg.insert_row(
                     ...
                     , X_DESCRIPTION     => 'Receipt for '||v_paytype||' '||v_property
...
------------------------------------------------------------------------------------
 ...
 begin
     ...
     update rnt_ledger_entries
     set debit_account  = v_pt_rule.debit_account
     ,   credit_account = X_CREDIT_ACCOUNT
     where ar_id = x_ar_id
     ...
     update_alloc(X_PAYMENT_DATE         => X_PAYMENT_DATE
                 ...
                 , X_AR_ID               => X_AR_ID
                 , X_PAYMENT_PROPERTY_ID => X_PAYMENT_PROPERTY_ID
                 , X_PAYMENT_TYPE_ID     => X_PAYMENT_TYPE
                 , X_DEBIT_ACCOUNT       => X_DEBIT_ACCOUNT
                 , X_CREDIT_ACCOUNT      => v_pt_rule.CREDIT_ACCOUNT);
 ...
 begin
     ...
     v_pt_rule := rnt_ledger_pkg.get_pt_accounts( X_BUSINESS_ID
      ...
      into v_paytype
      from rnt_payment_types
      where payment_type_id = x_payment_type;



     v_ledger_id := rnt_ledger_entries_pkg.insert_row(
                                 X_ENTRY_DATE      => X_PAYMENT_DUE_DATE
                               , X_DESCRIPTION     => 'Unscheduled receivable'||' '||v_paytype||' '||v_property
                               , X_DEBIT_ACCOUNT   => v_pt_rule.DEBIT_ACCOUNT
