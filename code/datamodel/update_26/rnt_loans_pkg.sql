...
begin
  ...
  if is_exists_acc_receivable(X_LOAN_ID) then
     RAISE_APPLICATION_ERROR(-20181, 'Cannot delete record. For loan exists accounts receivable.');
  end if; 
  if is_exists_acc_payable(X_LOAN_ID) then
     RAISE_APPLICATION_ERROR(-20182, 'Cannot delete record. For loan exists accounts payable.');
  end if;
