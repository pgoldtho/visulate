CREATE OR REPLACE PACKAGE RNTMGR.RNT_BUSINESS_UNITS_PKG AS
/******************************************************************************
...
CREATE OR REPLACE PACKAGE BODY RNTMGR.RNT_BUSINESS_UNITS_PKG AS
/******************************************************************************
   NAME:       RNT_BUSINESS_UNITS_PKG
   PURPOSE:
...
begin
   ...
   where exists (
                   select 1
                   from RNT_BUSINESS_UNITS
                   where BUSINESS_NAME = X_BUSINESS_NAME
                     and PARENT_BUSINESS_ID = X_PARENT_BUSINESS_ID
...
is
  x NUMBER;
begin
   if not check_unique(NULL, X_BUSINESS_NAME, X_PARENT_BUSINESS_ID) then
       RAISE_APPLICATION_ERROR(-20006, 'Sud business name business unit must be unique');                      
   end if;   
...
begin
   ...
   if not check_unique(X_BUSINESS_ID, X_BUSINESS_NAME, X_PARENT_BUSINESS_ID) then
        RAISE_APPLICATION_ERROR(-20006, 'Sub business name business unit must be unique');                      
   end if;   
...
CREATE OR REPLACE PACKAGE BODY RNTMGR.RNT_LOANS_PKG AS
/******************************************************************************
   NAME:       RNT_LOANS_PKG
   PURPOSE:
...
END RNT_PROPERTY_UNITS_PKG; 
/
CREATE OR REPLACE PACKAGE RNTMGR.RNT_SYS_CHECKSUM_REC_PKG AS
/******************************************************************************
...
CREATE OR REPLACE PACKAGE BODY RNTMGR.RNT_SYS_CHECKSUM_REC_PKG AS
/******************************************************************************
   NAME:       RNT_SYS_CHECKSUM_REC_PKG
   PURPOSE:
