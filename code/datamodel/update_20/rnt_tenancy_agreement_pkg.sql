...
begin
  if not is_exists_current_tenant(X_AGREEMENT_ID) then 
     RAISE_APPLICATION_ERROR(-20104, 'The agreement does not have a primary tenant.');
   end if;    
end;    
...
begin
   if is_exists_current_tenant(X_AGREEMENT_ID) then
     ...
     if x_agr_date is null then
         RAISE_APPLICATION_ERROR(-20105, 'The agreement has a primary tenant but the agreement start date is null.');     
     end if;  
   end if;
end;     
...
begin
   if is_exists_current_tenant(X_AGREEMENT_ID) then
     ...
     if x_agr_date > SYSDATE then
         RAISE_APPLICATION_ERROR(-20106, ' The agreement has a primary tenant but the agreement start date is in the future.');     
     end if;  
   end if;
end;     
...
begin
  ...
  where exists (select 1
   ...
   RAISE_APPLICATION_ERROR(-20107, 'The agreement dates overlap with another agreement.');
exception   
...
begin
     ...
     if x_end_date < trunc(SYSDATE) + 30 then
         RAISE_APPLICATION_ERROR(-20108, 'The agreement end date is less than 30 days in the future.');     
     end if;  
end;
...
begin
     ...
     if x_end_date < trunc(SYSDATE) + 60 then
         RAISE_APPLICATION_ERROR(-20109, 'The agreement end date is less than 60 days in the future.');     
     end if;  
end;
...
begin
     ...
     if x_end_date < SYSDATE then
         RAISE_APPLICATION_ERROR(-20110, 'The agreement has expired.');     
     end if;  
end;
...
begin
     ...
     if x_end_date < x_start_date then
         RAISE_APPLICATION_ERROR(-20111, 'The agreement end date is earlier than the agreement start date.');     
     end if;  
end;          
...
begin
  ...
  where exists (
   ...
   RAISE_APPLICATION_ERROR(-20012, 'Section 8 office has been selected by tenant pays is null.');
exception   
  when NO_DATA_FOUND then NULL;  
end;
...
begin
     ...
     if (x_amount is not null or x_type is not null or x_period is not null) and (x_amount is null or x_type is null or x_period is null) then
         RAISE_APPLICATION_ERROR(-20113, 'A value has been entered in one of the late fee fields but one of the other fields is null.');     
     end if;  
end;          
