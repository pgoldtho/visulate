create table tmp_tax_value as
 select c.name
    ,   u2.description
    ,      round(PERCENTILE_DISC(0.8) within group (order by s.tax_value/nvl(p.sq_ft, p.acreage)) /1, 2) a_median
    ,      round(PERCENTILE_DISC(0.5) within group (order by s.tax_value/nvl(p.sq_ft, p.acreage)) /1, 2) b_median
    ,      round(PERCENTILE_DISC(0.2) within group (order by s.tax_value/nvl(p.sq_ft, p.acreage)) /1, 2) c_median
    ,      count(*) sales_count
...
--    and (s.tax_value/nvl(p.sq_ft, p.acreage) < 25)
    ...
    group by c.name, u2.description having count(*) > 20
    order by c.name, u2.description;
