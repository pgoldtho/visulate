...
Properties in Cape Canaveral are generally considered less desirable than similar properties in Cocoa Beach to the south. Much of the residential real estate in both cities dates from the 1960&#39;s.&nbsp; A lot of the cheaper condos are converted multi-family properties.&nbsp; Typically 1 bed/ 1 bath units with less than 400 sq ft of living space.&nbsp; Zoning restrictions for most of the city limit the minimum term of a rental agreement.&nbsp; These are designed to limit the areas in Cape Canaveral where condos can be used as vacation rentals.</p>';
  ...
  function get_census_text( p_city     in varchar2
    ...
    v_table_header   varchar2(4000) :=
      '<table class="datatable"><caption>Economic Indicators</caption>'
    ||'<tr><th scope="col">Region</th>'
    ||'<th scope="col">Population<br/>2010</th>'
  ...
  begin
    ...
    for fl_rec in cur_content(12000) loop
     v_table_text :=  v_table_text||'<tr><th scope="row"><a href="/rental/visulate_search.php?REPORT_CODE=CITY&state=FL">Florida</a>'
                           ||'</th><td style="text-align:right">'||to_char(fl_rec.population_recent, '999,999,999')
    ...
    for county_rec in cur_content(p_cou) loop
     v_table_text :=  v_table_text||'<tr><th scope="row"><a href="/rental/visulate_search.php?REPORT_CODE=CITY&state=FL&county='
                           ...
                           ||'</th><td style="text-align:right">'||to_char(county_rec.population_recent, '999,999,999')
                           ...
                           ||'</td></tr>'
                           ||'<tr><th scope="row">'||v_city_rec.city
                           ||'</th><td style="text-align:right">'||to_char(v_city_rec.population_recent, '999,999,999')
      ...
      ||'<table class="datatable"><caption>'||v_city_rec.city||' Economic Activity</caption>'
      ||'<tr><th scope="col">Activity</th><th scope="col">Value (Millions)</th><th scope="col">Percentage</th></tr>'
      ||'<tr><th scope="row">Manufacturers shipments</th><td style="text-align:right">'
      ...
      ||'<tr><th scope="row">Merchant wholesaler sales</th><td style="text-align:right">'
      ...
      ||'<tr><th scope="row">Retail sales</th><td style="text-align:right">'
      ||to_char(v_city_rec.retail_sales/1000, '$999,999,999')||'</td>'
      ||'<td style="text-align:right">'
      ||to_char(v_city_rec.retail_sales/v_city_economy_size*100,'90.9')||'%</td></tr>'
      
      ||'<tr><th scope="row">Accommodation and food services sales</th><td style="text-align:right">'
      ...
      ||'<tr><th scope="row">Total</th><td style="text-align:right">'
...
---------------------------------------
       ...
       order by city) loop
         if a_rec.city = v_city_rec.city then
            v_table_text := v_table_text
               ||'<tr><th class="current" scope="row">'||a_rec.city
               ||'</th><td  class="current" style="text-align:right">'||to_char(a_rec.population_recent, '999,999,999')
            ...
            v_table_text := v_table_text
               ||'<tr><th scope="row"><a href="/rental/visulate_search.php?REPORT_CODE=CITY&state=FL&county='
               ||upper(replace(v_county_name, ' County'))||'&city='||upper(a_rec.city)||'">'||a_rec.city||'</a>'
               ||'</th><td style="text-align:right">'||to_char(a_rec.population_recent, '999,999,999')
      ...
      for c_rec in cur_content(v_region_list(i)) loop
        ...
        if v_region_list(i) = p_cou then
          v_table_text := v_table_text ||'<tr><th class="current" scope="row">'||c_rec.name
                           ||'</th><td class="current" style="text-align:right">'||to_char(c_rec.population_recent, '999,999,999')
          ...
          v_table_text := v_table_text ||'<tr><th scope="row"><a href="/rental/visulate_search.php?REPORT_CODE=CITY&state=FL&county='
                           ...
                           ||'</th><td style="text-align:right">'||to_char(c_rec.population_recent, '999,999,999')
  ...
  if v_ucode_summary('Total').property_count > 0 then
    if p_mode = 'table' then
      ...
      ||'<table class="datatable"><caption>'||v_display_name||' Real Estate by Usage</caption>'
