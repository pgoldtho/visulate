...
begin
  ...
  for x in (select TENANT_ID
      ...
      x_ar_id := RNT_ACCOUNTS_RECEIVABLE_PKG.INSERT_ROW( 
                     ...
                     , X_PAYMENT_PROPERTY_ID  => x_property_id
                   );
				   
     v_account := rnt_ledger_pkg.get_pt_accounts
               ( p_business_id      => x_business_id
               , p_payment_type_id  => RNT_ACCOUNTS_RECEIVABLE_PKG.PAYMENT_TYPE_RECOVERABLE
               , p_transaction_type => 'ARS');
			   
	 v_ledger_id := rnt_ledger_entries_pkg.insert_row
                   ( X_ENTRY_DATE        => x_action_date
                    , X_DESCRIPTION     => 'Recoverable Expense'
                    , X_DEBIT_ACCOUNT   => v_account.debit_account
                    , X_CREDIT_ACCOUNT  => v_account.credit_account
                    , X_PAYMENT_TYPE_ID => RNT_ACCOUNTS_RECEIVABLE_PKG.PAYMENT_TYPE_RECOVERABLE
                    , X_AR_ID           => x_ar_id
                    , X_AP_ID           => null
                    , X_PAY_ALLOC_ID    => null
...
begin
   ...
   if not check_unique(X_ACTION_ID, X_AGREEMENT_ID, X_ACTION_DATE, X_ACTION_TYPE) then
        RAISE_APPLICATION_ERROR(-20090, 'Action must be unique.');                      
   end if;   
   ...
   if x_old.RECOVERABLE_YN = 'Y' and X_RECOVERABLE_YN = 'N' then
      ...
      if NVL(x_alloc_payment, 0) > 0 then
        RAISE_APPLICATION_ERROR(-20091, 'Cannot update action record. Cause: find record about payment allocation.');
      end if;
      -- delete records 
   ...
   if x_old.RECOVERABLE_YN = 'Y' and X_RECOVERABLE_YN = 'Y' and NVL(X_ACTION_COST, 0) - NVL(x_old.ACTION_COST, 0) < 0 then
      ...
      if X_ACTION_COST < NVL(x_alloc_payment, 0) then
        RAISE_APPLICATION_ERROR(-20092, 'Cannot update action record. Cause: find record about payment allocation. Action cost cannot be less '||NVL(x_alloc_payment, 0)||'.');
      end if; 
   end if;
...
begin
   ...
   values(RNT_AGREEMENT_ACTIONS_SEQ.NEXTVAL, X_AGREEMENT_ID, X_ACTION_DATE, 
          X_ACTION_TYPE, empty_clob(), X_RECOVERABLE_YN, 
          X_ACTION_COST)
   returning ACTION_ID into x;

   update rnt_agreement_actions 
   set comments = x_comments
   where action_id = x;
...
begin
  x_alloc_payment := RNT_ACCOUNTS_RECEIVABLE_PKG.GET_MAX_ALLOC_FOR_AGR_ACTION(X_ACTION_ID);
  if NVL(x_alloc_payment, 0) > 0 then
      RAISE_APPLICATION_ERROR(-20093, 'Cannot delete action record. Cause: find record of payment allocation.');
  end if;
