...
/*
 Alerts: 
  ...
  6.    Expenses that do not have a corresponding row in the AP table.
...
function flag_uninvoiced_expense       return varchar2;
...
/******************************************************************************
   ...
   Ver        Date        Author           Description
   ---------  ----------  ---------------  ------------------------------------
   1.0        05.11.2007  Alexander         Created this package body.
   1.1        04.27.2010  Peter             Added uninvoiced expenses test to get_alerts 
******************************************************************************/
...
function flag_uninvoiced_expense        return varchar2 is begin return 'UNE'; end;
...
is
  cursor c_agr_expiried is
        ...
        where p.BUSINESS_ID = X_BUSINESS_ID
          ...
          and not exists (select 1
                          ...
                          where ta.UNIT_ID = pu.UNIT_ID
                            ...
                            and nvl(ta.end_date, sysdate + 1) > sysdate)
        order by p.ADDRESS1, pu.UNIT_NAME;
   ...
   cursor c_unpaid_balance is
    ...
    cursor c_uninvoiced_expenses is
        ...
        ,    rnt_property_units pu
        where p.business_id = X_BUSINESS_ID
        and p.property_id   = pe.property_id
...
begin
  ...
  -- Blocked toilet at 219 Grannis Ave Unit 1
  for x in c_uninvoiced_expenses loop
      add_rec(X_PROPERTY_ID       => x.property_id,
              X_UNIT_ID           => x.unit_id, 
              X_TENANT_ID         => NULL, 
              X_BUSINESS_ID       => X_BUSINESS_ID,
              X_RECORD_TYPE       => flag_uninvoiced_expense,
              X_PAYMENT_DUE_MONTH => to_char(x.event_date, 'YYYY'),
              X_DESCRIPTION       => x.description||' at '||x.address1||', '||x.unit_name,
              X_AGREEMENT_ID      => x.expense_id);
  end loop;             
  return ret;    
end get_alerts;
       ...
       noi.CAP_RATE_VALUE 
        from (
              ...
              (
                ...
                where ap.ap_id = pa.ap_id
                  and ap.BUSINESS_ID = X_BUSINESS_ID
                  and ap.payment_type_id not in (41, 59, 61) -- Building Purchase, Land Purchase or Loan Origination
              ...
              (
                ...
                from rnt_accounts_payable ap
                where BUSINESS_ID = X_BUSINESS_ID 
                and ap.payment_type_id not in (41, 59, 61)
                and ap.loan_id is null
                group by ap.PAYMENT_PROPERTY_ID
              ) invoiced,
              RNT_PROPERTIES p,
...
show errors package RNT_SUMMARY_PKG
show errors package body RNT_SUMMARY_PKG
