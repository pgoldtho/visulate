...
function REGISTER_ACCOUNT(X_USER_LOGIN_EMAIL RNT_USER_REGISTRY.USER_LOGIN_EMAIL%TYPE,
                          ...
                          X_TELEPHONE         RNT_USER_REGISTRY.TELEPHONE%TYPE := ''
...
X_ADMIN_EMAIL            constant varchar(60) := 'peter@goldthorp.com';
...
begin
  ...
  insert into RNT_USER_REGISTRY ( USER_REGISTRY_ID
                                , USER_LOGIN_EMAIL
                                , USER_PASSWORD
                                , USER_HASH_VALUE
                                , USER_NAME
                                ...
                                , LAST_UPDATE_DATE
                                , TELEPHONE) 
  values ( RNT_USER_REGISTRY_SEQ.NEXTVAL
         , X_USER_LOGIN_EMAIL
         , ENCRYPT_PASSWORD(X_USER_PASSWORD)
         , substr(ENCRYPT_PASSWORD(to_char(SYSDATE, 'RRRRMMDDHH24MISS')||'+'||X_USER_LOGIN_EMAIL), 1, 64)
         , X_USER_NAME
         , X_USER_LAST_NAME
         , X_INVITE_USER_ID
         , SYSDATE
...
is                          
  ...
  x_l1business_id    RNT_BUSINESS_UNITS.BUSINESS_ID%TYPE;
  x_l2business_id    RNT_BUSINESS_UNITS.BUSINESS_ID%TYPE;
...
begin
  ...
  x_user_id := INSERT_ROW
            (X_USER_LOGIN       => rec.USER_LOGIN_EMAIL,
             ...
             X_PRIMARY_PHONE    => rec.TELEPHONE,
             X_SECONDARY_PHONE  => NULL,
             X_IS_SUBSCRIBED_YN => 'Y',  
             X_AGREEMENT_DATE   => SYSDATE        
             ); 
   
  -- create business unit
  x_l1business_id := RNT_BUSINESS_UNITS_PKG.INSERT_ROW_AUTOMATIC(
                         X_BUSINESS_NAME => rec.USER_LOGIN_EMAIL||' Root',
                         X_PARENT_BUSINESS_ID => 0);
                         
  x_l2business_id := RNT_BUSINESS_UNITS_PKG.INSERT_ROW_AUTOMATIC(
                         X_BUSINESS_NAME => rec.USER_LOGIN_EMAIL||' Properties',
                         X_PARENT_BUSINESS_ID => x_l1business_id);

  -- assign role for user
  x_user_assign_id := 
                    RNT_USER_ASSIGNMENTS_PKG.INSERT_ROW(X_USER_ID     => X_USER_ID,
                                                        X_ROLE_ID     => X_MANAGER_OWNER_ROLE_ID,
                                                        X_BUSINESS_ID => x_l1business_id);
                    
  x_user_assign_id := 
                    RNT_USER_ASSIGNMENTS_PKG.INSERT_ROW(X_USER_ID     => x_user_id,
                                                        X_ROLE_ID     => X_ADVERTISE_ROLE_ID,
                                                        X_BUSINESS_ID => x_l2business_id);

  x_user_assign_id := 
                    RNT_USER_ASSIGNMENTS_PKG.INSERT_ROW(X_USER_ID     => x_user_id,
                                                        X_ROLE_ID     => X_BUYER_ROLE_ID,
                                                        X_BUSINESS_ID => x_l2business_id);
...
exception
  when NO_DATA_FOUND then
      RAISE_APPLICATION_ERROR(-20524, 'We were unable to find your registration entry.  Please check the '
                                    ||' link that you clicked on to get to this screen and verify it is complete.'
                                    ||' It is also possible that your account has already been created.  The '
                                    ||' create account link can only be used once.  Click on the Contact Us link at the '
                                    ||' top of the page if you need help.');  
end;                          
