...
AS
  ...
  c_module_name          CONSTANT varchar2(250) := 'Centroid';
  ...
  c_i_multi              CONSTANT INTEGER       := -20106;
  ...
  c_s_null_geometry      CONSTANT VARCHAR2(250) := 'Input geometry must not be null';
              ...
              FOR v_ring_no IN 1..v_num_rings LOOP
                  ...
                  If ( v_ring is not null ) Then
                      ...
                      If ( v_ring.sdo_elem_info(2) in (1003,2003) And v_coordinates.COUNT = 2 ) Then
                         ...
                         Pipe Row( T_Vector(3, Vertex2vertex(V_Coordinates(2),3),
...
*/
  ...
  Begin
    ...
    IF ( p_start = 2 ) THEN
       ...
       -- DEBUG dbms_output.put_line(p_seed_x || ': ' || v_mbr.sdo_ordinates(1) || ',' || p_geometry.sdo_ordinates(2) || ',' || p_geometry.sdo_ordinates(3) || ',' || p_geometry.sdo_ordinates(4) );
       IF ( p_seed_x <= v_mbr.sdo_ordinates(1) OR p_seed_x >= p_geometry.sdo_ordinates(3) ) THEN
          raise SEED_MBR;
       END IF;
    END IF;
                              ...
                              FROM (SELECT s.x,
                                      ...
                                      FROM (SELECT t.x,
                                              ...
                                              FROM (SELECT r.x,
                                                        ...
                                                        FROM (SELECT c.x,
                                                                   round(v.startCoord.x,p_round_x) as startX,
                                                                   round(v.startCoord.y,p_round_y) as startY,
                                                                   round(  v.endCoord.x,p_round_x) as endX,
                                                                   round(  v.endCoord.y,p_round_y) as endY
                                                              FROM (SELECT round(avg(p.x),p_round_x) as x
                                                                      FROM TABLE(mdsys.sdo_util.GetVertices(v_seed)) p
                                                                   ) c,
                                                                   TABLE(&&defaultSchema..CENTROID.GetVector(p_geometry)) v
                                                           ) r
  ...
  Begin
    ...
    CASE WHEN v_gtype IN (1,5)
         ...
         WHEN v_gtype in (3,7)
         THEN v_geometry := centroid_a(v_geometry,p_start,NULL,p_round_x,p_round_y,p_round_z);
  ...
  Begin
    If ( p_dimarray is null ) Then
     raise_application_error(&&defaultSchema..CONSTANTS.c_i_null_diminfo,&&defaultSchema..CONSTANTS.c_s_null_diminfo,true);
    End If;
